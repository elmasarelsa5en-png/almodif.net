...
import { useEffect } from 'react';import React, { useState, useMemo, useEffect } from 'react';

import { useRouter } from 'next/navigation';import { motion, AnimatePresence } from 'framer-motion';

import { 

export default function RestaurantRedirect() {  Utensils, ArrowLeft, Star, Plus, ShoppingCart, 

  const router = useRouter();  CreditCard, Clock, Crown, Timer,

    Flame, Snowflake, Cookie, Croissant, ChefHat, User, Phone, Home, CheckCircle, Loader2

  useEffect(() => {} from 'lucide-react';

    router.replace('/guest-app');import { Button } from '@/components/ui/button';

  }, [router]);import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

import { Badge } from '@/components/ui/badge';

  return (import { useRouter } from 'next/navigation';

    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';

      <div className="text-white text-xl">جاري التحويل...</div>import { getMenuItemsByCategory, subscribeToMenuItems, type MenuItem } from '@/lib/firebase-data';
